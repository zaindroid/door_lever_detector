...
class t2d2t3d:
 ...
 #       self.map_grey = cv2.cvtColor(self.map_img, cv2.COLOR_BGR2GRAY)
    ...
    def get_box_voxel(self, bbox, cloud):
        print("Inside get_box_voxel function")
        print("Bounding box:", bbox)
        rob_coords = np.array([0, 0, 0])
        cld = self.cloud_msg_to_ndarray(cloud, fields=['x', 'y', 'z'])
        print("Cloud shape:", cld.shape)
        padd = 5
        data_croped = cld[bbox[0][1]:bbox[1][1]-padd,bbox[0][0]:bbox[1][0]-padd,:]
        print("Cropped data shape:", data_croped.shape)
        print("Cropped data:")
        print(data_croped)
        # Remove NaN values from the cropped data
        nan_mask = np.isnan(data_croped)
        data_croped_clean = data_croped[~np.any(nan_mask, axis=2)]
        # data_croped_clean = data_croped[~np.isnan(data_croped).any(axis=1)]
        # Remove NaN values from the cropped data


        print("Cleaned cropped data shape:", data_croped_clean.shape)
        print("Cleaned cropped data:")
        print(data_croped_clean)
    ...
    def plane_remove(self,whole,object_pose,padd=0.4):
        print("Inside plane_remove function")
    ...
    def detect_obstacle(self, whole, obj_clus, rob_coords, no_plane, tolerance=100, viz=False):
        print("Inside detect_obstacle function")
        print("Robot coordinates:", rob_coords)
        print("Visualization flag:", viz)
        if no_plane == None:
            no_plane = whole
...
#        return valid_points_new
    ...
    def plan_grab_poses(self, bbox, ros_trans, viz=False):
        print("Inside plan_grab_poses function")
        print("Bounding box:", bbox)
        print("ROS transformations:", ros_trans)
        print("Visualization flag:", viz)
        for a in range(5):
            try:
                #bbox = [[x1,y1],[x2,y2]]
